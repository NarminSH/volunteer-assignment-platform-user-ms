...
def import_data(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    all_users_in_db = db.query(models.Volunteers).all()

    database_ids = []
    for user in all_users_in_db:
        database_ids.append(user.candidate_id)

    if duplicate_ids_excel == []: 
        for key in all_users_in_excel:
            candidate_id = key["Candidate - ID"]
            if candidate_id not in database_ids:
        ...
        return {"status": status.HTTP_400_BAD_REQUEST, "result": "Duplicate ID", "message": all_ids_excel}
